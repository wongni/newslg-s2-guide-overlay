...
export function UpgradeCalculator() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 backdrop-blur-sm pt-20 pb-8"
          onClick={(e) => {
            if (e.target === e.currentTarget) setIsOpen(false);
          }}
        >
          <div className="w-full max-w-sm mx-4 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 overflow-hidden">
            {/* Modal Header */}
            ...
            <div className="px-5 py-4 space-y-3 overflow-y-auto">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={calculate}
                  className="flex-1 px-4 py-2 rounded-lg text-sm font-medium text-white bg-amber-600 hover:bg-amber-500 transition-colors"
                >
                  계산하기
                </button>
                <button
                  onClick={resetForm}
                  className="px-4 py-2 rounded-lg text-sm font-medium transition-colors bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 text-zinc-600 dark:text-zinc-300"
                >
                  초기화
                </button>
              </div>
